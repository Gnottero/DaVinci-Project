...
execute as @e[tag=TreeEnabled] at @s if block ~ ~1 ~ dropper{Items: [{Slot: 4b, id: "minecraft:oak_sapling", Count: 1b}]} run tag @s add Oak
execute as @e[tag=TreeEnabled,tag=Oak] at @s run replaceitem block ~ ~1 ~ container.4 air
execute as @e[tag=TreeEnabled,tag=Oak] run data merge entity @s {ArmorItems:[{},{},{},{id:"minecraft:firework_rocket",Count:1b,tag:{CustomModelData:10444444}}]}
execute as @e[tag=TreeEnabled,tag=Oak] run tag @s remove TreeEnabled

execute as @e[tag=TreeEnabled,nbt={Fire:0s}] run data merge entity @s {Fire:23000s}
execute as @e[tag=Oak,nbt={Fire:0s}] run data merge entity @s {Fire:23000s}
...
execute as @e[tag=Oak] at @s if block ~ ~1 ~ air run setblock ~ ~2 ~ air destroy
execute as @e[tag=Oak] at @s if block ~ ~2 ~ air run setblock ~ ~1 ~ air destroy
execute as @e[tag=Oak] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run summon item ~ ~ ~ {Item: {id: "minecraft:item_frame", Count: 1b, tag: {CustomModelData: 10424242, display: {Name: "{\"text\":\"Mini Tree Farm\",\"color\":\"green\",\"bold\":\"true\"}"}, EntityTag: {Tags: ["Tree"]}}}}
execute as @e[tag=Oak] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run summon item ~ ~ ~ {Item:{id:"minecraft:oak_sapling",Count:1b}}
execute as @e[tag=Oak] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run kill @e[type=item,nbt={Item:{id:"minecraft:dropper",Count:1b}},distance=..2]
execute as @e[tag=Oak] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run kill @e[type=item,nbt={Item:{id:"minecraft:glass",Count:1b}},distance=..2]
execute as @e[tag=Oak] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run kill @s
