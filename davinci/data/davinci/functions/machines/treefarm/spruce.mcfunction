...
execute as @e[tag=TreeEnabled] at @s if block ~ ~1 ~ dropper{Items: [{Slot: 4b, id: "minecraft:spruce_sapling", Count: 1b}]} run tag @s add Spruce
execute as @e[tag=TreeEnabled,tag=Spruce] at @s run replaceitem block ~ ~1 ~ container.4 air
execute as @e[tag=TreeEnabled,tag=Spruce] run data merge entity @s {ArmorItems:[{},{},{},{id:"minecraft:firework_rocket",Count:1b,tag:{CustomModelData:5}}]}
execute as @e[tag=TreeEnabled,tag=Spruce] run tag @s remove TreeEnabled

execute as @e[tag=TreeEnabled,nbt={Fire:0s}] run data merge entity @s {Fire:23000s}
execute as @e[tag=Spruce,nbt={Fire:0s}] run data merge entity @s {Fire:23000s}
...
execute as @e[tag=Spruce] at @s if block ~ ~1 ~ air run setblock ~ ~2 ~ air destroy
execute as @e[tag=Spruce] at @s if block ~ ~2 ~ air run setblock ~ ~1 ~ air destroy
execute as @e[tag=Spruce] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run summon item ~ ~ ~ {Item: {id: "minecraft:item_frame", Count: 1b, tag: {CustomModelData: 10424242, display: {Name: "{\"text\":\"Mini Tree Farm\",\"color\":\"green\",\"bold\":\"true\"}"}, EntityTag: {Tags: ["Tree"]}}}}
execute as @e[tag=Spruce] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run summon item ~ ~ ~ {Item:{id:"minecraft:spruce_sapling",Count:1b}}
execute as @e[tag=Spruce] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run kill @e[type=item,nbt={Item:{id:"minecraft:dropper",Count:1b}},distance=..2]
execute as @e[tag=Spruce] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run kill @e[type=item,nbt={Item:{id:"minecraft:glass",Count:1b}},distance=..2]
execute as @e[tag=Spruce] at @s if block ~ ~1 ~ air if block ~ ~2 ~ air run kill @s
